...
- **管理者トグルは isAdmin（運営者ログイン）時のみ表示**。参加者・未ログイン・Google参加者には管理画面の存在を見せない。運営者は参加者画面フッターの控えめな「運営者ログイン」入口からメール/PWログイン → isAdmin 判明でトグル出現（LoginScreen/AdminNoPermission には「参加者ページに戻る」導線あり）
